...
export function usePracticeSession() {
  ...
  const handlePreviousExercise = useCallback(() => {
    const { previous, remaining } = takePreviousExercise(exerciseHistory);

    if (!previous) {
      return;
    }

    setExerciseHistory(remaining);
    setSession((current) => {
      if (!current) {
        return current;
      }
      ...
      return {
        ...
        latestResult: null,
        hintMessages: [],
        learningPath: selectLearningPathExercise(
          current.learningPath,
          previous
        )
      };
    });
    setCode(previous.starterCode);
  }, [exerciseHistory]);

  const handleOpenExercise = useCallback(async (exerciseId: string) => {
    if (!session || exerciseId === session.exercise.id) {
      return;
    }
    ...
    try {
      ...
      setExerciseHistory((history) =>
        appendExerciseHistory(history, session.exercise, exercise)
      );
      setSession((current) => {
        if (!current) {
          return current;
        ...
        return {
          ...
          latestResult: null,
          hintMessages: [],
          learningPath: buildNextLearningPath(
            current.learningPath,
            exercise
          )
        };
  ...
  return {
    ...
    canGoPrevious: exerciseHistory.length > 0,
    weakKcs,
    latestHint,
    handleRun,
    handleSubmit,
...
function selectLearningPathExercise(
  ...
  const selected: LearningPathItem = {
    id:
      items.find((item) => item.title === exercise.title)?.id ??
      `path-${exercise.id}`,
    title: exercise.title,
    kcCode: exercise.kcTags[0]?.code ?? "python_basics",
    state: "current",
    etaMinutes: exercise.estimatedMinutes,
    difficulty: exercise.difficulty
  };
  const remaining = items
    .filter((item) => item.title !== exercise.title)
    .map((item) =>
      item.state === "current"
