...
def run_code_with_judge0(request: ExecutionRunRequest) -> ExecutionRunResponse:
    response_data = execute_code_request(request)

    return normalize_judge0_response(response_data, request)


def execute_code_request(
    request: ExecutionRunRequest,
    runner_mode: str | None = None,
) -> dict[str, Any]:
    mode = (runner_mode or settings.code_runner_mode).strip().lower()

    if mode == "judge0":
        return submit_to_judge0(build_judge0_payload(request))

    if mode in {"mock", "local"}:
        return run_code_locally(request)

    raise ExecutionServiceError(f"Unsupported CODE_RUNNER_MODE: {mode}")


def run_code_locally(request: ExecutionRunRequest) -> dict[str, Any]:
    if request.language != "python":
        raise ExecutionServiceError("Only python is supported in the MVP runner")
    ...
    return {
        ...
        "compile_output": "",
        "message": "",
        "time": f"{runtime_seconds:.4f}",
        "memory": 0,
        "status": {
            "id": 3 if accepted else 11,
            "description": "Accepted" if accepted else "Runtime Error",
        },
    }
