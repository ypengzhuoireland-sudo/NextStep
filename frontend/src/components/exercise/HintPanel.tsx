      ...
      <CardContent className="space-y-4 p-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[1, 2, 3].map((level) => {
            ...
            const loadingThisLevel = hintingLevel === typedLevel;

            return (
              <Button
                key={level}
                variant={typedLevel === activeLevel ? "default" : "secondary"}
                size="sm"
                disabled={isHinting}
                onClick={() => onRequestHint(typedLevel)}
              >
                <span
                  aria-hidden="true"
                  className="relative inline-grid h-3.5 w-3.5 place-items-center"
                  translate="no"
                >
                  <Lightbulb
                    className={`absolute h-3.5 w-3.5 transition-opacity ${
                      loadingThisLevel ? "opacity-0" : "opacity-100"
                    }`}
                  />
                  <Loader2
                    className={`absolute h-3.5 w-3.5 animate-spin transition-opacity ${
                      loadingThisLevel ? "opacity-100" : "opacity-0"
                    }`}
                  />
                </span>
